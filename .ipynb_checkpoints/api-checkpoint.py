...
def reformat(input_string, expected_keys=['content', 'emotion'], emotion_list=['sad', 'cheerful', 'whisper', 'default', 'friendly']):
    try:
        ...
        for part in parts:
            ...
            result[key.strip()] = remove_punctuation(value.strip().lower())
...
def chatbot(user, audio):
    ...
    try:
        with open(f'cache/{user}_data.json', 'r') as json_file:
    ...
    if my_dict is None:
        my_dict = [{'role': 'user',
                    'content': 'You are a chatgot for emotional therapy. Now, you need to answer user input by few sentence. '
                               'You need to reply in this format: content: your reply content. | emotion: the emotion will be used for the tts model. You should change it based on the conversation scene.'
                               'here are emotions you can use: [sad, cheerful, whisper, default, friendly]. Let me know whenyou are ready.'},
                   {'role': 'assistant', 'content': 'I am ready now.'},
                   {'role': 'user', 'content': 'This is a test. Reply anything with default emotion.'},
                   {'role': 'assistant', 'content': 'content: yes. | emotion: default.'}]
    if audio is not None:
        ...
        with open(f'cache/{user}_data.json', 'w') as json_file:
